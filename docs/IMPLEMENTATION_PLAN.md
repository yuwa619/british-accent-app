...
- **Phase 10 complete:** Playwright smoke/e2e coverage, accessibility landmark fixes, feedback page heading, protected retention purge route, provider QA checklist, retention purge plan, known limitations, beta launch checklist, README updates, and final mock-mode validation.
- **Phase 11 complete:** Real-environment readiness assets: protected system-health endpoint with safe booleans only, `.env.local.example`, Supabase verification SQL, Vercel deployment guide, preview-targetable Playwright config, and updated provider/deployment runbooks. Real provider tests still require supplied staging keys.
- **Phase 12 complete:** Staging deployment QA readiness: staging Supabase runbook, staging Vercel deployment runbook, staging QA results document, updated Supabase verification queries, and documentation of current blockers. Real staging QA was not run because credentials/project link were unavailable.
...
- **Phase 11 deployment QA:** `GET /api/system/health` is protected by `MAINTENANCE_SECRET` and returns safe configuration booleans only. `docs/VERCEL_DEPLOYMENT_GUIDE.md` now defines preview/production setup, env vars, provider QA order, and post-deploy smoke tests. `docs/SUPABASE_VERIFICATION_QUERIES.sql` provides SQL editor checks for migrations, RLS flags, policies, seed data, bucket privacy, triggers, and indexes.
- **Phase 12 staging QA:** `docs/STAGING_SUPABASE_RUNBOOK.md` defines Supabase staging setup, migrations, seed, SQL checks, two-user RLS testing, Storage ownership checks, and retention smoke. `docs/STAGING_DEPLOYMENT_RUNBOOK.md` defines Preview env vars, Vercel deployment steps, protected health checks, preview Playwright command, and provider test order. `docs/STAGING_QA_RESULTS.md` records that real staging QA remains blocked until Supabase credentials and a valid Vercel project/token are supplied.
- **Post-MVP next steps:** Complete live provider-key QA, run Supabase staging QA with two users, verify retention purge in staging, add Stripe webhook handling before enabling paid plans, complete accessibility/manual mobile QA, invite a small beta cohort, monitor feedback/errors, and plan V1 improvements such as transcoding, stronger acoustic analysis, richer admin tooling, and realtime roleplay only after MVP validation.
